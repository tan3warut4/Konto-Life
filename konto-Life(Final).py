...
class Main():
     ...
     def slary2(self):
          ...
          self.main.resizable(width=FALSE, height=FALSE)
          self.y = Entry(self.main, justify='center')
     ...
     def run_resultuse(self):
          self.main = Tk()
          self.main.geometry("500x400+500+100")
          self.result = '%.2f' % (float(self.slary - self.value)/day_left())
          #--Background--#
     ...
     def run_last(self):
         ...
         self.t = Entry(self.main, justify='center')
         self.t.place(x=295, y=295)
         Button(self.main, image = bt_img4, bg = '#29a3a2', relief= 'flat', command = self.get_use).place(x=420, y=340)
         Label(self.main, text =  str(day_left()), font=("Helvetica", 30), bg = '#29a3a2', fg = 'white').place(x=300, y=60)
     ...
     def use(self):
         ...
         self.can = '%.2f' % ((float(self.left) - float(self.got))/(day_left() - 1))
